        ...
        <div className="max-w-4xl mx-auto px-4 pb-8 md:pb-16 lg:pb-20">
            <div className="relative bg-base-100 rounded-3xl shadow-2xl border border-secondary/10 overflow-visible flex flex-col">
                ...
                <div className="flex justify-center -mt-20 mb-4 z-10">
                    <div className="rounded-3xl shadow-lg border-4 border-base-100 bg-gradient-to-br from-secondary/10 to-base-200 p-2">
                        <img
                            src={food.food_img}
                            alt={food.food_name}
                            className="rounded-2xl w-60 h-60 object-cover transition-transform duration-500 hover:scale-105 bg-base-100"
                        />
                    </div>
                </div>
                <div className="flex flex-col gap-4 p-6 md:p-10 pt-2">
                    ...
                    {/* Info row */}
                    <div className="flex flex-wrap gap-4 text-sm md:text-base text-accent mb-2 justify-center border-b border-dashed border-secondary/20 pb-4">
                        <span><span className="font-bold text-primary">Origin:</span> {food.food_origin}</span>
                        <span><span className="font-bold text-primary">Quantity:</span> {food.quantity}</span>
                        <span><span className="font-bold text-primary">Price:</span> ${food.price}</span>
                        <span><span className="font-bold text-primary">Sold:</span> {food.purchase_count}</span>
                    </div>
                    {/* Details */}
                    <div className="text-base md:text-lg text-gray-700 mb-2 whitespace-pre-line text-center leading-relaxed">
                        {food.details}
                    </div>
                    {/* Ingredients */}
                    <div>
                    ...
                    <div>
                        ...
                        <div className="text-sm md:text-base text-gray-700 whitespace-pre-line text-center leading-relaxed">
                    ...
                    <div className="mt-8 flex flex-col md:flex-row gap-4 justify-center">
                        <Link to="/all-foods" className="btn btn-outline btn-secondary rounded-3xl font-semibold shadow-md">Back to All Foods</Link>
                        <Link to={`/food-purchase/${food._id}`} className="btn btn-secondary rounded-3xl font-bold text-white shadow-lg hover:scale-105 transition-transform">Purchase</Link>
                    </div>
                </div>
            </div>
        </div>
